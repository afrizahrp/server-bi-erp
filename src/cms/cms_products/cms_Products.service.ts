...
@Injectable()
export class cms_ProductsService {
  constructor(private prisma: PrismaService) {}

  async create(
    cms_CreateProductDto: Cms_CreateProductDto,
  ): Promise<Cms_ResponseProductDto> {
    const product = await this.prisma.imc_Products.create({
  ...
  async findBySlug(
    ...
    const product = await this.prisma.imc_Products.findFirst({
  ...
  async update(
    ...
    const product = await this.prisma.imc_Products.findUnique({
      where: { company_id_id: { id, company_id } },
    });
    if (!product) {
      throw new NotFoundException(`Product with ID ${id} not found`);
    }
    const cms_UpdatedProduct = await this.prisma.imc_Products.update({
      where: { company_id_id: { id, company_id } },
